...
const Cart = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const res = await axios.get(
        "https://book-store-vdd0.onrender.com/api/v1/get-user-cart",
        { headers }
      );
  ...
  const deleteItem = async (bookid) => {
    const response = await axios.put(
      `https://book-store-vdd0.onrender.com/api/v1/remove-from-cart/${bookid}`,
      {},
      { headers }
    );
  ...
  const PlaceOrder = async () => {
    try {
      const response = await axios.post(
        'https://book-store-vdd0.onrender.com/api/v1/place-order',
        { order: Cart },
        { headers }
      );
      alert(response.data.message);
